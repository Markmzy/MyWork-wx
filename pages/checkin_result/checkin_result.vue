<template>
	<view class="page">
		...
		<view class="checkin-report">
			...
			<view class="report-title">
				<text class="days-small">{{ weekStart }}</text>
				<text class="days-small">------</text>
				<text class="days-small">{{ weekEnd }}</text>
			</view>
			<view class="calendar-container">
				<view class="calendar" v-for="one in weekCheckIn" :key="one">
					...
					<text class="result dark" v-if="one.type == '节假日'">休息</text>
				</view>
			</view>
		</view>
	</view>
	...
	onShow: function() {
		...
		that.ajax(that.url.searchTodayCheckIn, 'GET', null, function(resp) {
			...
			(that.name = result.name),
				...
				(that.weekStart = result.weekStart),
				(that.weekEnd = result.weekEnd);
		});
	},
	onBackPress() {
		uni.redirectTo({
